...
| `list_team_leave_requests(p_org_id, p_status) → table` | **175** (supersedes 155/135/130) | DEFINER | Row-filtered inside query: admin OR manages that row's user | List requests of a given status for the caller's team/org; 175 appends `created_at` for the "Requested on" column | leave.js |
